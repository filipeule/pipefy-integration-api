...
2. **`POST /webhooks/pipefy/card-updated`** — Simula o recebimento de um webhook do Pipefy quando um card é atualizado. Aplica a regra de prioridade baseada no patrimônio do cliente, garante **idempotência** por `event_id` e simula o envio da mutation de atualização ao Pipefy (`updateCardField`).
